...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('vps_usage_summaries', function (Blueprint $table) {
            ...
            $table->integer('avg_number_ip_address')->nullable()->default(1);
            
            // Metadata
            $table->dateTime('created_at')->nullable()->useCurrent();
            $table->dateTime('updated_at')->useCurrentOnUpdate()->nullable()->useCurrent();
            
            // Composite index cho query nhanh
            $table->unique(['user_id', 'instance_id', 'period_start', 'period_type']);
            $table->index(['period_start', 'period_type']);
        });
    }
