...
@section('content')
    <div class="content-wrapper p-2 pt-3">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">Chỉnh sửa Media Item</div>
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('media.items.update', $item) }}" method="POST">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="mt-2">
                                    @foreach ($folders as $folder)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="folders[]" id="folder{{ $folder->id }}" value="{{ $folder->id }}"
                                                {{ in_array($folder->id, old('folders', $selectedFolders)) ? 'checked' : '' }}>
                                ...
                                <select class="form-control @error('primary_folder_id') is-invalid @enderror" id="primary_folder_id" name="primary_folder_id">
                                    ...
                                    @foreach ($folders as $folder)
                                        <option value="{{ $folder->id }}"
                                            {{ old('primary_folder_id', $primaryFolder ? $primaryFolder->id : '') == $folder->id ? 'selected' : '' }}>
                                            {{ $folder->name }}
                                        </option>
                                    @endforeach
                                </select>
