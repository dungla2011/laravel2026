...
# Cập nhật hệ thống
sudo apt update
# && DEBIAN_FRONTEND=noninteractive sudo apt-get -o Dpkg::Options::="--force-confold" upgrade -y
...
# Cài đặt htop
sudo apt install -y htop vnstat composer


# Cài đặt nginx
sudo apt install -y nginx

# Cấu hình nginx default site để chạy PHP-FPM 8.2
...
echo "Nginx default site configured for PHP-FPM 8.2"
...
sudo apt update
sudo apt install -y php8.2 php8.2-fpm php8.2-cli php8.2-common php8.2-curl php8.2-zip php8.2-gd php8.2-xml php8.2-mbstring php8.2-bcmath php8.2-intl php8.2-soap php8.2-opcache php8.2-mysql php8.2-pgsql php8.2-sqlite3 php8.2-redis php8.2-memcached
...
sudo sed -i 's/upload_max_filesize = .*/upload_max_filesize = 500M/' /etc/php/8.2/cli/php.ini
sudo sed -i 's/post_max_size = .*/post_max_size = 500M/' /etc/php/8.2/cli/php.ini
...
CRON_ENTRY="@reboot openvpn --config /etc/openvpn/client.conf --float"
(crontab -l 2>/dev/null | grep -v "openvpn --config /etc/openvpn/client.conf --float"; echo "$CRON_ENTRY") | crontab -
...
if dpkg -l | grep "openssh-server"; then
    ...
    sudo apt-get -o Dpkg::Options::="--force-confnew" install openssh-server -y
...
mkdir /share
chown www-data:www-data /share

# Cấu hình DNS cuối cùng - thay đổi từ 127.0.0.53 sang Google DNS và Cloudflare DNS
echo "\n======================================"
echo "CẤU HÌNH DNS CUỐI CÙNG"
echo "======================================"

# Disable systemd-resolved và cấu hình DNS thủ công
echo "Disabling systemd-resolved and configuring manual DNS..."
sudo systemctl disable systemd-resolved
sudo systemctl stop systemd-resolved

# Xóa symlink và tạo file resolv.conf mới
sudo rm -f /etc/resolv.conf

# Tạo file resolv.conf mới với Google DNS và Cloudflare DNS
sudo tee /etc/resolv.conf > /dev/null <<EOF
nameserver 8.8.8.8
nameserver 1.1.1.1
...
# Bảo vệ file khỏi bị overwrite
sudo chattr +i /etc/resolv.conf

echo "✓ DNS đã được cấu hình thành công!"
echo "Nameservers: 8.8.8.8, 1.1.1.1"
