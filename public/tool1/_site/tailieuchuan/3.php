...
foreach ($mq AS $item) {
    ...
    if(str_contains($item->content, 'https://tailieuchuan.mytree.vn/test_cloud_file?fid=')){

        preg_match('/(?<=fid=)\d+/', $item->content, $matches);
        $number = $matches[0]; // Số number trong URL

        $newNumber = $midx[$number];

        echo "<br/>\n Found file ID : $number --> $newNumber";

        $f1 = \App\Models\FileUpload::find($number);
        $f2 = \App\Models\FileUpload::find($newNumber);
...
//            continue;
            ...
            $change = 1;
            echo "<br/>\n Will change...";
            $newCont = preg_replace('/(?<=fid=)\d+/', $newNumber, $item->content);

            echo "<br/>\n Txt1: <span style='color: blue'>". htmlspecialchars($item->content) ."</span>";
            echo "<br/>\n Txt2 <span  style='color: red'>". htmlspecialchars($newCont)."</span>";
//            $item->content = $newCont;
//            $item->save();
        }
