...
//            $sheet->setCellValue([$colIndex + 1, $rowIndex + 1], $cellValue);
//
//        }
//    }
//

    $ignore = explode(',', "image_list,signature,status,user_id,created_at,parent_id,parent_extra");

    $colIndex = 1;
    $mfImportDb = $meta->getFieldToImportExcel();
    foreach ($mfImportDb as $field => $nameAndSize) {
        $sheet->setCellValue([$colIndex++, 1], $field);
    ...
    foreach ($mfImportDb as $field => $nameAndSize) {
        $sheet->setCellValue([$colIndex++, 2], $nameAndSize['name']);
    }

// Đặt chế độ bảo vệ cho hai dòng đầu tiên
    $spreadsheet->getActiveSheet()->getStyle('A1:B2')->getProtection()
...
// Freeze the first two rows
    ...
    $sheet->getStyle('A1:Z2')->getFill()
        ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
        ->getStartColor()->setARGB('a9b5db');
