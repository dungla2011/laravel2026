...
//                this.moveKhongCoConThiMoveSatAnhEm(r)
                        ...
                        if (createNew) {
                            ...
                            // if(this.setPid == 'sn792546')
                            {
                                leftBtn = "<div id='id_node_move_left_" + obj.id + "' data-id='" + obj.id +
                                    "' title='(Tinh chỉnh) Dịch trái - Có thể ấn phím Trái Trên bàn phím' class='node_move_left_btn btn btn-neutral'>" +
                                    " < </div>";
                                rightBtn = "<div id='id_node_move_right_" + obj.id + "' data-id='" + obj.id +
                                    "' title='(Tinh chỉnh) Dịch phải - Có thể ấn phím Phải Trên bàn phím' class='node_move_right_btn btn btn-neutral'>" +
                                    " > </div>";
                            }
    ...
    drawLineMarried(obj) {
        ...
        px = x1 + (w1 + this.spaceBetweenCellX) * deltaCol
        py = y1 + h1 / 2
        strLine += " " + px + "," + py
        ...
        if (mMarried.length > 1) {
            for (let mr of mMarried) {
                ...
                px = x1 + cc * (this.spaceBetweenCellX + w1)
        ...
        callback: function (key, options) {
            ...
            if (key == 'view_list_child') {
                if (clsTreeTopDownCtrl.doingNodeObj.married_with)
                    window.location.href = '/member/tree-mng?seby_s4=' + clsTreeTopDownCtrl.doingNodeObj.married_with;
                else
                    window.location.href = '/member/tree-mng?seby_s4=' + idItem;
            }
            ...
            if (key == 'view_list_brother') {
                if (clsTreeTopDownCtrl.doingNodeObj)
                    window.location.href = '/member/tree-mng?seby_s4=' + clsTreeTopDownCtrl.doingNodeObj.parent_id;
            }
        ...
        buttons: [
            ...
            click: function () {
                ...
                if (1) {
                    ...
                    delete objBannerPost.old_id;
                    delete objBannerPost.old_image_list;
                    delete objBannerPost.old_tree_id;
                    delete objBannerPost.old_tree_nodes_xy;
                    delete objBannerPost.old_user_id;
...
$(document).keydown(function(event) {
    if(0)
        if (event.ctrlKey){
            ...
            let tre1 = clsTreeTopDownCtrl.getInstanceSvgById('svg_grid');
            if(tre1._panZoomTiger.isZoomEnabled())
                tre1._panZoomTiger.disableZoom();
            else
                tre1._panZoomTiger.enableZoom()

            if(tre1._panZoomTiger.isPanEnabled())
                tre1._panZoomTiger.disablePan()
            else
                tre1._panZoomTiger.enablePan()
        }
});
