...
function assertValidConfig(object $location, string $label): void
{
    ...
    foreach ($required as $f) {
        if (! isset($location->{$f})) {
            exit("Missing value for $f in $label".PHP_EOL);
        }
    }

    if (isset($location->ssh) && ! isset($location->ssh->host)) {
        exit("SSH tunnel is specified for $label, but no host is set.".PHP_EOL);
    }
}
...
function mysql_sync(array|object|null $config): void
{
    ...
    if (! isset($config->source)) {
        exit("Configuration is missing 'source' database options.".PHP_EOL);
    }
    if (! isset($config->dest)) {
        exit("Configuration is missing 'dest' database options.".PHP_EOL);
    }
    ...
    if (getCounts($statements) > 0) {
        ...
        //		$r = prompt("Do you want to deploy the changes to the destination, dump the SQL modification commands to a file or cancel? [D]eploy to destination, [s]ave to file, [c]ancel",
        //            allowedResponses:['D', 's', 'c']);

        echo "\n\n= {$config->source->database} -> {$config->dest->database} \n\n";

        //Nếu có apply all, và out = như vậy, thì apply all
        if (clsConfigTmpSyncDbGlx::$apply_for_all_last_if_the_same == $out) {
            echo "\n Apply all... $out";
            context::mysql_transaction($dest)->do(function ($dest) use ($source) {
                sync($source, $dest, false);
            });
        } else {
            $r = getch("Do you want to deploy the changes to the destination, dump the SQL modification commands to a file or cancel?\n
[D]eploy to destination, [s]ave to file, [l] apply for all left if the same, [c] Cancel:");
            ...
            //Nếu apply all các bảng còn lại:
            if ($r == 'l') {
                clsConfigTmpSyncDbGlx::$apply_for_all_last_if_the_same = $out;
                context::mysql_transaction($dest)->do(function ($dest) use ($source) {
                    sync($source, $dest, false);
                });
            }
            ...
            if (clsConfigTmpSyncDbGlx::$apply_for_all_last_if_the_same) {
                getch('...apply for all left if the same ...');
            } else {
                getch('...');
            }
        }
...
function sync($source, $dest, bool $dryRun): array
{
    ...
    foreach ($new as $tblName => $cols) {
        ...
        //lad sync Show index:
        $indexes = $source->query("SHOW INDEX FROM `$tblName`");
        // Tạo các câu lệnh ALTER TABLE để tạo lại các chỉ mục trên bảng đích
        foreach ($indexes as $index) {
            $sql = "ALTER TABLE $tblName ADD ";
            $sql .= $index['Non_unique'] ? 'INDEX ' : 'UNIQUE INDEX ';
            $sql .= "`{$index['Key_name']}` ";
    ...
    if (count($dropped) > 0) {
        foreach ($dropped as $tblName => $cols) {
            ...
            if (in_array($tblName, clsConfigTmpSyncDbGlx::$ignore_drop_table)) {
                echo "\n Ignore drop table $tblName !!!";
    ...
    foreach ($existing as $tblName => $master_cols) {
        ...
        if (count($alter) > 0) {
            ...
            //Lad:
            $command = str_replace('current_timestamp()', 'CURRENT_TIMESTAMP', $command);
            $command = str_replace("'current_timestamp()'", 'CURRENT_TIMESTAMP', $command);
