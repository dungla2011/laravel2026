...
function authenticateWebDav() {

    $validUsername = 'admin';
    $validPassword = '11112222';
...
//    }
    ...
    $pw0 = (sha1($password. $user->id));
//    die("UP = $user->id / $username / $password, $pw0 / $user->password");
    if($user && $pw0 == $user->password)
        return $user->getId();
...
class CustomPutPlugin extends DAV\ServerPlugin {
    ...
    function putDone($path, DAV\IFile $node) {
        ...
        ///////////////////////////////////////////////////////
        //Xem file đã tồn tại chưa Thì move file vào đây
            ...
            if($file = \App\Models\FileUpload::find($fid)){
                ...
                if($file->name == basename($fileUpDone) && $fileCloud = \App\Models\FileCloud::find($file->cloud_id)){
                    ...
                    if($fileCloud->save() && $file->save())
                    {
                        ...
                        touch($fileUpDone);
                        ol3(" Update content file old -> new : (fid: $fid) $fileUpDone -> $fileCloud->file_path");
                        return;
                    }
                }
            }
            ...
            if ($file = \App\Models\FileUpload::find($fid)) {
                if ($fileCl = \App\Models\FileCloud::find($file->cloud_id))
                    if (file_exists($fileCl->file_path)){
                        return $this->getFileContent($file->name, $fileCl->file_path, $fileCl->size);
...
//        return '';
    ...
    private function getFileContent($name, $filePath, $size) {

        ol3("---getFileContent ... $filePath ,  $name, $size");
        $start = 0;
        $length = $size;
...
//        header('Content-Type: application/octet-stream');
        ...
        $content = '';
        while ($length > 0) {
            $readSize = min($blockSize, $length);
            $content .= fread($file, $readSize);
            $length -= $readSize;
        }
...
//        return 1;
        ...
        ol3(" *** Error get size $fileFid / $this->path / or uploading? fileExit = " . file_exists($this->path));
        return 0;
...
//            // Lấy kích thước file từ stream
        ...
        if(file_exists($newPath) && $fileSize){
            $fileUpDone = $newPath;
        }else{
            $fileUpDone = $upDir . '/upload_dav_' .$this->uid."-". microtime(1);
            file_put_contents($fileUpDone, $data);

            if(!file_exists($fileUpDone) || !filesize($fileUpDone)){
                ol3(" *** Error: File not found $fileUpDone or zero size");
                return;
            }
            $fileSize = filesize($fileUpDone);
        }
...
//        die($fileUpDone . ' Size: ' . filesize($fileUpDone));
        ...
        ol3(" --- upload file to folder: $folderID , $fileUpDone, $fileSize");

        $std = new stdClass();
        $std->file_name = $name;
        $std->file_size = $fileSize;
        $std->file_path_local_upload_ = $fileUpDone;
        $std->folder_id = $folderID;
        ...
        try{
            $ret = \App\Http\ControllerApi\FileUploadControllerApi::uploadStatic($std,1);
            ol3("Upload done1? " . serialize($ret));
        }
        catch (Throwable $e) { // For PHP 7
...
//        }
        ...
        if(!$idRet)
        {
            ol3(" *** Can not upload : $fileUpDone ". filesize($fileUpDone));
            loi("Error: Can not upload: ". serialize($ret));
        }
        else{
            ol3(" ID OK = $idRet");
            touch($fileUpDone);
...
//            die();
    ...
    public function getChild($name)
    {
        ...
        if (!file_exists($path)) {
            ol3("File with name $path could not be located");
