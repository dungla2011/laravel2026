...
//            180 => '180 Giây',
    ...
    function extraHtmlIncludeEdit0($objData = null,  $v2 = null,  $v3 = null)
    {
  ...
  ?>
        ...
        <script src="/js/monitor-timeline.js"></script>

        <!-- HTML đơn giản -->
        <div id="my-timeline" data-auto-init="monitor-timeline"
             data-api-url="/api/monitor-graph/uptime"
             data-monitor-name="<?php  echo $objData->getName(); ?>"
             data-monitor-id="<?php  echo $objData->getId(); ?>"
             data-url-edit="#"
             data-period="24h"
             data-height="20">
        </div>
...
//        die();
        $str = "";
        $mmSelectForMobile = [];
        $mmSelectForMobile[0] = __('monitor.select_alert_type');
        $tmp = [];
...
";
            ...
            $str = "<p> " . __('monitor.no_alert_config') . ". <a href='/member/monitor-config' target='_blank'>" . __('monitor.create_new') . "</a></p>";
        }
    ...
    public function extraJsInclude()
    {
        ...
        <script>
            ...
            $(function() {
                ...
                let lastClickId = 0;
                function successOrNot(val){
                    console.log("successOrNot = ", val);
                    if(val === 0){
                        $(`.enable[data-id=${lastClickId}]`).removeClass('fa-toggle-on').addClass('fa-toggle-off')
                        console.log(" off status ");
                    }
                }

                $(".change_status_item").on('click', function () {
                    console.log("Change status item21 ...");

                    //Tìm cha gần nhất của nó có data-id='id' và lấy ra id
                    let id = $(this).closest("div.divTable2Cell").data('id');
                    lastClickId = id;
                    if(id){
                        clsTableMngJs.saveOneIdTable(id, successOrNot)  // Truyền function reference, không phải string
                    }
                });
                ...
                function stopAlertToNHour(nHour){
                    ...
                    $.post(url, data, function(response){
                        if(response.code && response.code > 0){
                            ...
                            toastr.error(response.message || "Error update!");
                        }
...
<!--            </ul>-->
    ...
    function extraJsIncludeEdit($objData = null)
    {
        ...
        $(document).ready(function() {
            ...
            if(getCurrentActionMethod() == 'create'){
            ?>
                $(".divTable2Row.allow_alert_for_consecutive_error").hide();
                $(".divTable2Row.enable").hide();

                console.log(" Set defaul value monitor: ");

                $("div.divTable2Row.stopTo").hide();
                $(".divTable2Row._mount_alert_id ").hide();
                $(".sl_option.check_interval_seconds").val('<?php echo $this->getDefaultValue('check_interval_seconds') ?>');
                $(".sl_option.type").val('<?php echo  $this->getDefaultValue('type') ?>');

                $('input[data-field="type"]').attr("value", '<?php echo  $this->getDefaultValue('type') ?>');
                $('input[data-field="type"]').prop("value", '<?php echo  $this->getDefaultValue('type') ?>');

                $(".fa.fa-toggle-off.change_status_item.enable").removeClass('fa-toggle-off').addClass('fa-toggle-on');
            ...
            $(".status_edit_one .change_status_item").on('click', function () {
                console.log("Change status item22 ...");
