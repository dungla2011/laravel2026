...
//                die();
    ...
    function updateChangeLog($mVal = [], $uid = -1, $mField = [])
    {
        ...
        if($mVal){
            foreach ($mVal AS $field => $val){
                if(strlen($val) > 64000){
                    $mVal[$field] = "[Data over 10k, ignore]";
                }
            }
            $m1['change_log'] = serialize($mVal);
        }
        else{
            $mLog = $this->toArray();
            foreach ($mLog AS $field => $val){
                if(strlen($val) > 64000){
                    $mLog[$field] = "[Data over 10k, ignore]";
                }
            }
            if($mLog['log'] ?? '')
                unset($mLog['log']);
            $m1['change_log'] = serialize($mLog);
        }

        if($mField)
        foreach ($m1 AS $field => $val){
            if(!in_array($field, $mField)){
                unset($m1[$field]);
            }
        }
...
//    }
    ...
    /**
     * Backup table to ZIP file using mysqldump
     * Usage: VpsUsage::backupTable('/var/glx/weblog/backup_table/vps_usages-2026-03-06-10.zip');
     *
     * @param string $fileZipPath Full path to zip file to create
     * @return array ['success' => bool, 'message' => string, 'file_size' => int]
     */
    public static function backupTable($fileZipPath)
    {
        try {
...
//            echo "</pre>";
            ...
            $zipDir = dirname($fileZipPath);
            if (!is_dir($zipDir)) {
                @mkdir($zipDir, 0755, true);
            }

            // Create temp SQL file
            $sqlTempFile = $zipDir . '/' . $tableName . '_temp.sql';

            // Build mysqldump command
            $cmd = sprintf(
                'mysqldump -h %s -u %s -p%s -P %s %s %s > %s 2>&1',
            ...
            if (!file_exists($sqlTempFile)) {
                return [
                    'success' => false,
                    'message' => 'SQL dump file not created',
                    'file_size' => 0
                ];
            }

            // Check if zip file already exists
            if (file_exists($fileZipPath)) {
                @unlink($sqlTempFile);
                return [
                    'success' => false,
                    'message' => 'Backup file already exists',
                    'file_size' => 0
                ];
            }
            ...
            // Add SQL file to zip
            $zip->addFile($sqlTempFile, $tableName . '.sql');
            $zip->close();
            ...
            // Get file size
            $fileSize = file_exists($fileZipPath) ? filesize($fileZipPath) : 0;

            return [
                'success' => true,
                'message' => "Backup created successfully: {$tableName}",
                'file_size' => $fileSize
            ];
